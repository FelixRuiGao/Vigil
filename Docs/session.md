...
- 支持两种模式：
  - inline mode：直接在 tool call 参数中传 `operations`
  - file mode：通过 `file` 参数指定 `.yaml` 文件路径（相对于 SESSION_ARTIFACTS），从中读取 `operations`
- file mode 成功后，自动压缩 show_context 到 summarize_context 之间的中间步骤（read_file/write_file/edit_file on summary file），插入合成 summary entry

...
describe("Textarea - Scroll Tests", () => {
  ...
  describe("Viewport clamp after content-shrinking edit", () => {
    // Regression: the deferred `_pendingViewportClamp` used to clamp the
    // viewport with moveCursor=true. When a content-shrinking edit (e.g.
    // Cmd+Backspace at the start of the last logical line) ran while the
    // composer was scrolled, the clamp dragged the caret up onto the previous
    // logical line. The clamp must only pull the scroll back into bounds, never
    // relocate the caret.
    it("keeps the caret on its own line after deleting the line-start newline while scrolled", async () => {
      const filler = Array.from({ length: 12 }, (_, i) => `line ${i}`).join("\n")
      const secondToLast = "the second to last line"
      const lastLine = "the very last line"
      const { textarea: editor } = await createTextareaRenderable(currentRenderer, renderOnce, {
        initialValue: `${filler}\n${secondToLast}\n\n${lastLine}`,
        width: 40,
        height: 5, // small -> content overflows and scrolls
        wrapMode: "word",
        scrollMargin: 0,
      })

      const lastRow = editor.editBuffer.getLineCount() - 1 // the last line (preceded by an empty line)
      editor.editBuffer.setCursor(lastRow, 0) // caret before the first char of the last line
      await renderOnce()

      // precondition: the composer is actually scrolled
      expect(editor.editorView.getViewport().offsetY).toBeGreaterThan(0)

      editor.deleteToLineStart() // delete the preceding (empty) line's newline
      await renderOnce() // runs the deferred _pendingViewportClamp

      const cursor = editor.logicalCursor
      // caret stayed on its own (merged) line, NOT dragged up to `secondToLast`
      expect(cursor.row).toBe(lastRow - 1)
      expect(cursor.col).toBe(0)
      const lines = editor.plainText.split("\n")
      expect(lines[cursor.row]).toBe(lastLine)
      expect(lines[cursor.row - 1]).toBe(secondToLast)

      editor.destroy()
    })

    it("control: same edit behaves identically when not scrolled", async () => {
      const secondToLast = "the second to last line"
      const lastLine = "the very last line"
      const { textarea: editor } = await createTextareaRenderable(currentRenderer, renderOnce, {
        initialValue: `${secondToLast}\n\n${lastLine}`,
        width: 40,
        height: 20, // tall enough: no scroll
        wrapMode: "word",
        scrollMargin: 0,
      })

      const lastRow = editor.editBuffer.getLineCount() - 1
      editor.editBuffer.setCursor(lastRow, 0)
      await renderOnce()
      expect(editor.editorView.getViewport().offsetY).toBe(0)
      ...
      expect(cursor.row).toBe(lastRow - 1)
      expect(cursor.col).toBe(0)
      expect(editor.plainText.split("\n")[cursor.row]).toBe(lastLine)

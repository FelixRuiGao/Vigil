 ...
 */
function openRouterConfigName(modelId: string): string {
...
async function stepConfigureProvider(provider: ProviderPreset): Promise<SelectedModel[]> {
  ...
  if (provider.id === "openrouter") {
    ...
    return selectedModelIds.map((modelId) => ({
      configName: openRouterConfigName(modelId),
  ...
  return [{ configName, provider: provider.id, model: selectedModelId, apiKey }];
}
...
async function stepSelectDefault(models: SelectedModel[]): Promise<string> {
  ...
  return await select({
    ...
    choices: models.map((m) => ({
      name: `${m.configName} (${m.provider}/${m.model})`,
      value: m.configName,
    })),
  });
}
...
export async function runInitWizard(): Promise<WizardResult> {
  ...
  // Copy bundled templates and skills
